...
centos8中安装： `sudo dnf install perf`
...
centos8 安装： sudo dnf install sysstat
...
![image-20201012152114050](C:\Users\MUJI\AppData\Roaming\Typora\typora-user-images\image-20201012152114050.png)
...
sysctl 用于运行时配置内核参数，这些参数位于/proc/sys目录下。sysctl配置与显示在/proc/sys目录中的内核参数.linux系统在启动时，systemc-sysctl会根据优先级，依次读取 ① /etc/sysctl.d/\*.conf ② /run/sysctl.d/\*.conf ③ /user/lib/sysctl.d/*.conf 文件读取sysctl内核参数
